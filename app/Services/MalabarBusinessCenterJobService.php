...
/**
 * Fetches jobs from Malabar Business Center (Kozhikode / Calicut).
 * MBC does not operate a dedicated job-board, so we try their known careers
 * pages and use a generic JSON-LD + heuristic link extractor to pull any
 * vacancy listings available.
 */
class MalabarBusinessCenterJobService
{
    private const BASE = 'https://www.malabarbc.com';
    ...
    public function search(string $role, string $keyword = '', int $limit = 30): array
    {
        ...
        try {
            ...
            if (empty($jobs)) {
                return ['jobs' => [], 'error' => 'No current vacancies could be read from the Malabar Business Center website. Check malabarbc.com directly for the latest openings.'];
            }

            return ['jobs' => array_slice($jobs, 0, $limit), 'error' => null];

        } catch (\Throwable $e) {
            Log::error('MalabarBusinessCenterJobService fetch failed', ['error' => $e->getMessage()]);
            return ['jobs' => [], 'error' => 'Could not fetch Malabar Business Center jobs: ' . $e->getMessage()];
        }
    }
    ...
    private function extractJobs(string $html, string $pageUrl, string $role, string $keyword, int $limit): array
    {
        ...
        foreach ($links as $m) {
            ...
            $seen[$key] = true;
            $jobs[] = [
                'company'         => 'Malabar Business Center',
                'job_title'       => $text,
                'location'        => 'Malabar Business Center, Kozhikode, Kerala',
                ...
                'job_url'         => $href,
                'apply_url'       => $href,
                'source'          => 'Malabar Business Center',
                'apply_type'      => 'link',
                'description'     => '',
                'posted'          => null,
                'employer_logo'   => null,
    ...
    private function normalizePosting(array $jp, string $pageUrl): array
    {
        ...
        $company = is_array($org) ? ($org['name'] ?? 'Malabar Business Center') : (is_string($org) ? $org : 'Malabar Business Center');
